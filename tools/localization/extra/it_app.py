...
APP = {
    ...
    'A small system service for the two switches that need administrator rights: closed-display mode below, and AWDL pausing in Gaming & Streaming. Without it, macOS asks for your password once per app run. With it, both are instant and silent: macOS asks once, when you approve the helper under Login Items, and the approval survives restarts and updates. It can only flip those specific switches, everything it changes is restored if Keepresso quits or crashes, and you can remove it here at any time. It also puts the keepresso command-line tool on your PATH (Homebrew installs already have it). After removal, System Settings can keep showing a stale Login Items row until macOS refreshes its list; the status shown here is the real one.': 'Un piccolo servizio di sistema per i due interruttori che richiedono i privilegi di amministratore: la modalità a schermo chiuso qui sotto e la pausa di AWDL in Gaming e streaming. Senza di esso, macOS chiede la password una volta per ogni esecuzione dell’app. Con esso, entrambi sono istantanei e silenziosi: macOS chiede una volta, quando approvi l’assistente negli Elementi di login, e l’approvazione sopravvive a riavvii e aggiornamenti. Può azionare solo quegli interruttori specifici, tutto ciò che modifica viene ripristinato se Keepresso esce o va in crash, e puoi rimuoverlo da qui in qualsiasi momento. Aggiunge inoltre lo strumento a riga di comando keepresso al tuo PATH (le installazioni via Homebrew lo hanno già). Dopo la rimozione, Impostazioni di Sistema può continuare a mostrare una riga obsoleta negli Elementi di login finché macOS non aggiorna l’elenco; lo stato mostrato qui è quello reale.',
